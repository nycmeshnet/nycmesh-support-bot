...
def handle_support_request(app, config, user_id, channel_id, message_ts, manual_number=None, at_member = True):
    ...
    if at_member:
        text = f"This is a reply to <@{user_id}>! It looks like your email is {user.email} {'and' if user.network_number else 'but'} your network number {'is ' + nn_to_map_url(user.network_number) if user.network_number else 'could not be found'}. {f'Here is a diagnostics report to help our volunteers:{linesep}(The report will appear after approximately 40s.)' if user.network_number else ''}"
    else:
        text = f"The network number {'is ' + nn_to_map_url(user.network_number) if user.network_number else 'could not be found'}. {f'Here is a diagnostics report to help our volunteers:{linesep}(The report will appear after approximately 40s.)' if user.network_number else ''}"
